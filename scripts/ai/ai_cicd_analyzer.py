...
# We use this function to overwrite a regular repo URL to one which uses a Token for authentification
...
# Function for AI
def communicate_with_ai(api_endpoint, ai_api_key, file_content, action_type):
    request_headers = {
        "Content-Type": "application/json",
...
def main():
    ...
    if pr_create == "0":
        ...
        try:
            ...
            # Connect with AI
            response_text = communicate_with_ai(api_endpoint, ai_api_key, file_content, action_type)
    ...
    # We notify a user we can not create it
        ...
        print("A new PR can`t be created since the branch created by AI-Handler alredy exist")
    ...
    # The case if we need a new PR and there is no branch with the name <source_branch>-ai-handler
        ...
        # If the file path/name was set incorrectly we post a message in PR comments                
            ...
            error_message = "A target file can`t be overwriten. Please check the file path and try again."
            post_github_comment(owner, repo_name, pull_request_number, github_access_token, error_message, as_text=True)
            print(f"Error: {str(ex)}")
        ...
        created_pull_request = create_pull_request(github_access_token, owner, repo_name, source_branch, ai_handler_branch)
